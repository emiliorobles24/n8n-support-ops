...
| [IT Help Bot](workflows/it-help-bot.json) | Self-service deflection | A chat agent strictly grounded in help-center articles: cites the article it used, leads with the urgent path on security issues, and when the help center has no answer it says so and drafts the ticket instead of guessing. Ships with n8n's chat UI for demos; swap the trigger for Slack in production. |
| [Fleet Health Check](workflows/fleet-health-check.json) | Device posture | Daily sweep across Jamf, Kandji, and Intune: encryption, EDR presence, MDM check-in recency, OS-vs-target, disk space. Severity mirrors blast radius, every failure ships with its next action, and the digest separates the weekly score from today's to-do list. |
